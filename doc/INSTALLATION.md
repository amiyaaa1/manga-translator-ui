...
## 安装方式一：使用安装脚本（⭐ 推荐，无需预装 Python）

脚本会自动完成所有配置，并支持一键更新。

> ⚠️ **网络提示**：下载过程需要从 GitHub 拉取代码，网络不好建议开代理。
> 💡 **新特性**：无需预装 Python，脚本会自动下载便携版 Python 3.12

### 前提条件

- **无需预装 Python**：脚本会自动下载便携版 Python 3.12
...
**2.1 下载/解压便携版 Python 3.12**
- ✓ 如果已存在 `Python-3.12.12` 文件夹，直接使用
- ✗ 如果不存在：
  - 提供下载源选择：GitHub 官方 或 gh-proxy 镜像（国内推荐）
  - 自动下载 `python-3.12.12-win64.7z`（约 25MB）
  - 自动解压（如无 7-Zip 会自动下载便携版 7-Zip）
  - 解压完成后自动删除压缩包
