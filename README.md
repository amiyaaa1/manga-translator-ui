...
- 后端处理默认允许 **5** 路并发。你可以在 Zeabur 面板设置 `ZEABUR_MAX_CONCURRENCY`（或 `MT_MAX_CONCURRENCY`）环境变量即时调整上限。
...
2. **访问入口**：打开 `http://<主机或域名>:8000/`（或 `/console`）。首次进入会出现身份选择框：
   ...
   - **游客模式**：无需密钥，展示极简白色页面，提供单张图片上传、目标语言/预设/检测器/OCR 模型选择及开始翻译按钮；现已加入「ai断句」开关（原「关闭自动换行」），访客也能决定是否使用 AI 断句。
...
- 开启 ai 断句：关闭自动换行，文本行由 AI 翻译结果自行断句，适合保留原文段落结构或处理较长对话气泡。
- 关闭 ai 断句：启用自动换行，渲染器会根据气泡宽度重新排版并优化换行，更利于常规漫画气泡的均衡排版。
