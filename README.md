...
- Zeabur 默认禁止 `executable stack`，会导致基于 `ctranslate2` 的离线翻译器（Sugoi/Jparacrawl、M2M100）无法加载。容器会自动跳过这些翻译器，请在云端改用其他翻译器（如 `--translator nllb` / `--translator qwen2` 或云端 GPT/翻译服务）。
